...
public class RoundRobinProcessor extends SchedulingProcessor {
	...
	public RoundRobinProcessor(HashMap<String, ProcessInfo> processesInfo,
			int quantum) {
		super(processesInfo);
	...
	public void createSchedule() {
		HashMap<String, ProcessInfo> tempList = new HashMap<String, ProcessInfo>(
			processesInfo);
		...
		while (!rrQueue.isEmpty() || !tempList.isEmpty()){
			...
			for (Entry<String, ProcessInfo> e : tempList.entrySet()) {
				if (e.getValue().arrival <= currentTime) {
					if(currentArrivals.size() == 0){
						currentArrivals.add(new AbstractMap.SimpleEntry(e.getKey(), e.getValue().arrival));
					}
					else{
						int i = 0;
						for(; i < currentArrivals.size(); i++){
							// if this processes arrival is prior to list item
							if(e.getValue().arrival < currentArrivals.get(i).getValue())
							{
								break;
							}
						}
						// add to list in order of arrival (at index i)
						currentArrivals.add(i, new AbstractMap.SimpleEntry(e.getKey(), e.getValue().arrival));
					}
				}
			}
			...
			for(Entry<String, Integer> e: currentArrivals){
				...
				(e.getKey(), tempList.get(e.getKey()).length);
				// add to RR queue
				rrQueue.add(entry);
